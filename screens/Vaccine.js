...
export default function Vaccine({ navigation }) {
  ...
  React.useEffect(() => {
    fetch(
      `https://corona.lmao.ninja/v3/covid-19/vaccine/coverage/countries/${flag}?lastdays=all&fullData=true`
    )
      .then((res) => res.json())
      .then((obj) => {
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.box}>
          ...
          >
            ...
            >
              Most recent number of vaccinations:
            </Text>
            <Text style={{ fontFamily: "NunitoSans_700Bold", fontSize: 20 }}>
              {formatNumber(vaccinesDaily[vaccinesDaily.length - 1])}
            </Text>
          </View>
        </View>
      </Card>
      ...
      <Text style={{ textAlign: "center", fontFamily: "NunitoSans_700Bold" }}>
        Graph of vaccinations over time
      </Text>
    </View>
  );
}
